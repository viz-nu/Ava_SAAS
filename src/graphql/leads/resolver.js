...
export const leadResolvers = {
    Query: {
        fetchleadsTemplates: async (_, { limit = 10, page = 1, templateId, id, isActive }, context, info) => {
            ...
            const leadsTemplates = await LeadTemplate.find(filter).select(projection).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).populate({ path: 'business', select: nested.business }).populate({ path: 'createdBy', select: nested.createdBy });
        ...
        fetchLeads: async (_, { limit = 10, page = 1, templateId, id, status }, context, info) => {
            ...
            const leads = await Lead.find(filter).select(projection).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).populate({ path: 'business', select: nested.business }).populate({ path: 'createdBy', select: nested.createdBy });
            return leads;
        }
    },
    Mutation: {
        createLeadTemplate: async (_, { LeadTemplateInput }, context, info) => {
            ...
            const newLeadTemplate = await LeadTemplate.create({ name, description, fields, isActive: isActive !== undefined ? isActive : true, business: context.user.business, createdBy: context.user._id }).select(projection).populate({ path: 'business', select: nested.business }).populate({ path: 'createdBy', select: nested.createdBy });
        ...
        updateLeadTemplate: async (_, { id, LeadTemplateInput }, context, info) => {
            ...
            const updatedLeadTemplate = await LeadTemplate.findByIdAndUpdate(id, updateData, { new: true, runValidators: true }).select(projection).populate({ path: 'business', select: nested.business }).populate({ path: 'createdBy', select: nested.createdBy });
        ...
        createLead: async (_, { LeadCreateInput }, context, info) => {
            ...
            const newLead = await Lead.create({ template: templateId, data: validatedData, status: status || 'new', notes, business: context.user.business, createdBy: context.user._id }).select(projection)
        ...
        bulkCreateLeads: async (_, { dataList }, context, info) => {
            ...
            const newLeads = await Lead.insertMany(validatedDataList).select(projection).populate({ path: 'business', select: nested.business }).populate({ path: 'createdBy', select: nested.createdBy }).populate({ path: 'template', select: nested.template });
            return newLeads;
        },
        updateLead: async (_, { id, LeadCreateInput }, context, info) => {
            ...
            const updatedLead = await Lead.findByIdAndUpdate(id, { data: validatedData, status: status || 'new', notes }, { new: true, runValidators: true }).select(projection).populate({ path: 'business', select: nested.business }).populate({ path: 'createdBy', select: nested.createdBy }).populate({ path: 'template', select: nested.template });
