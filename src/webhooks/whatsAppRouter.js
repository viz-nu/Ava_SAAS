...
import { downloadAndTranscribeAudio, getMediaUrl, sendWAMessage } from "../utils/WA.js";
              ...
              if (value.messages && Array.isArray(value.messages)) {
                ...
                for (const message of value.messages) {
                  ...
                  if (message.type === "text" && message.text) {
                    ...
                    // Get the media URL using the WhatsApp API
                    const mediaUrl = await getMediaUrl({token:process.env.AVAKADO_WABA_TOKEN,mediaId});
                    if(mediaUrl) userMessageText = await downloadAndTranscribeAudio({token:process.env.AVAKADO_WABA_TOKEN,mediaUrl, mediaId,openAiKey:process.env.OPEN_API_KEY});
