...
async function processUserMessage(chatId, userMessage, bot, agentDetails, message) {
    ...
    await Message.create({ business: agentDetails.business._id, query: JSON.stringify(userMessage), response: JSON.stringify(result.finalOutput), conversationId: conversation._id, responseTokens: { model: agentDetails.personalInfo.model ?? null, usage } });
